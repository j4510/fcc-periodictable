...
# MAIN_MENU() {
  ...
  echo "$ELEMENT" | while IFS=\| read ATOMIC_NUMBER ATOMIC_MASS MELTING_POINT_CELSIUS BOI SY NA TYPE
      do
        echo  "The element with atomic number $ATOMIC_NUMBER is $NA ($SY). It's a $TYPE, with a mass of $ATOMIC_MASS amu. $NA has a melting point of $MELTING_POINT_CELSIUS celsius and a boiling point of $BOI celsius."
      done
  fi
